...
func (t *Translator) processHTTPListenerXdsTranslation(tCtx *types.ResourceVersionTable, httpListeners []*ir.HTTPListener) error {
	for _, httpListener := range httpListeners {
		...
		for _, httpRoute := range httpListener.Routes {
			...
			xdsCluster := buildXdsCluster(httpRoute.Name, httpListener.EnvoyProxy, httpRoute.Destinations, httpListener.IsHTTP2, true /* isStatic */)
			...
			for i, mirror := range httpRoute.Mirrors {
				...
				if cluster := findXdsCluster(tCtx, mirrorClusterName); cluster == nil {
					mirrorCluster := buildXdsCluster(mirrorClusterName, httpListener.EnvoyProxy, []*ir.RouteDestination{mirror}, httpListener.IsHTTP2, true /* isStatic */)
		...
		if err := createJwksClusters(tCtx, httpListener); err != nil {
			return err
		}
	}
	return nil
...
func processTCPListenerXdsTranslation(tCtx *types.ResourceVersionTable, tcpListeners []*ir.TCPListener) error {
	for _, tcpListener := range tcpListeners {
		...
		xdsCluster := buildXdsCluster(tcpListener.Name, tcpListener.EnvoyProxy, tcpListener.Destinations, false /*isHTTP2 */, true /* isStatic */)
		tCtx.AddXdsResource(resourcev3.ClusterType, xdsCluster)
...
func processUDPListenerXdsTranslation(tCtx *types.ResourceVersionTable, udpListeners []*ir.UDPListener) error {
	for _, udpListener := range udpListeners {
		...
		xdsCluster := buildXdsCluster(udpListener.Name, udpListener.EnvoyProxy, udpListener.Destinations, false /*isHTTP2 */, true /*isStatic */)
		tCtx.AddXdsResource(resourcev3.ClusterType, xdsCluster)
