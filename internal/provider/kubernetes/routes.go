			...
			for i := range rule.Filters {
				...
				if filter.Type == gwapiv1.HTTPRouteFilterRequestMirror {
					...
					case egv1a1.KindHTTPRouteFilter:
						...
						resourceTree.Append(httpFilter)
					default:
						extRefFilter, ok := resourceMap.extensionRefFilters[key]
						if !ok {
							r.log.Error(
